...
- 2026-02-21: **E28-BugFix: Frontend scheduling path consolidation (in progress, PRs pending).** Two drag/drop bugs fixed: (1) stale-closure reset — rescheduled tasks disappeared from calendar and reappeared as unscheduled; (2) silent sidebar drop crash — fake DragEvent without `preventDefault` caused TypeError. Fixes applied to `frontend/` only — no backend or contracts IPC changes needed (commands already implemented in E24). Changes: `services/backend.ts` now exports `calendarScheduleTask` + `calendarRescheduleEvent` wrappers; `hooks/useWeekDashboard.ts` uses `tasksRef`/`eventsRef` live refs for stable callbacks, `handleDayflowEventUpdate` routes through `calendarRescheduleEvent`, new `handleExternalDrop` function bypasses DragEvent API for sidebar drops, `calendarItems` sourced exclusively from `events` (single source of truth — `scheduledTasks` dual-path removed); `views/WeekDashboard.tsx` `onExternalDrop` delegates directly to `handleExternalDrop`; 15 tests green in `useWeekDashboard.test.ts`. Contracts `002_IPC_WIRING_MATRIX.md` updated with [E28-BugFix] note. Traceability `docs/traceability.md` updated with E28-BugFix row + two Known Gaps closed.
