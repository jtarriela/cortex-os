...
| **FR-013** | **FE:** `types.ts` (Meal, Recipe), `views/Meals.tsx`, `services/dataService.ts` (getMeals, addMeal, updateMeal, deleteMeal, getRecipes, addRecipe, updateRecipe, deleteRecipe). **ADR:** ADR-0002 | Weekly planner + recipe library. **Phase 0.5 gap CLOSED:** Meals/Recipes CRUD extracted to `dataService.ts`; `Meals.tsx` now uses `useEffect` + service calls; recipe image upload calls `updateRecipe`. Not in original vision. | `frontend/tests/dataService.smoke.test.ts` (FR-013 Meals/Recipes CRUD ✅), `backend/tests/meals.rs` |
...
| **FR-027** | **BE:** `crates/integrations/google_calendar` (OAuth + list/create calendar + event insert + incremental sync), `crates/app/src/lib.rs` (Integrations IPC sync routine). **FE:** `views/Settings.tsx` (auth/connect + calendar selection + sync trigger), `views/WeekDashboard.tsx` (event source badges + month toggle). **ADR:** ADR-0014 | **PARTIAL Phase 2** — OAuth connect, calendar listing, and `integrations_trigger_sync` are wired. Sync now pulls inbound events to local `calendar_event` pages and pushes outbound `sync_external` events to the dedicated Cortex calendar. Remaining gaps: outbound update/delete reconciliation and richer conflict handling. | `frontend/tests/calendar_sync.spec.ts`, `frontend/tests/settings_integrations.spec.tsx` |
...
| **2** Alpha Domains | Tasks + Calendar + Projects functional; Google Calendar integration partial | FR-001, FR-002, FR-003, FR-004, FR-015, FR-026, FR-027 (partial) | [#3](https://github.com/jtarriela/cortex-os/issues/3) |
