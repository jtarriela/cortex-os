...
| **FR-027** | **BE:** `crates/integrations/google_calendar` (Google Calendar adapter), `crates/app/src/lib.rs` (Settings IPC). **FE:** `views/Settings.tsx` (Integrations panel), `views/WeekDashboard.tsx` (badges, Month toggle). **ADR:** ADR-0014 | **IMPLEMENTED Phase 2** — Two-way Google Calendar sync stubs + UI logic. Connects via Settings. Drag-to-schedule sets `syncExternal` flag. Week/month view toggle added. | `frontend/tests/calendar_sync.spec.tsx` |
