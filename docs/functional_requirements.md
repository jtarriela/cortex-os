...
| ID | Requirement |
|----|-------------|
| **FR-001** | **Task management:** Users shall be able to create, view, update and delete tasks. Tasks have a title, optional description, status (`TODO`, `DOING`, `BLOCKED`, `DONE`, `ARCHIVED` — see ADR-0008), priority (`HIGH`, `MEDIUM`, `LOW`, `NONE`), optional due date, optional scheduled date with start time and duration, optional project reference, optional linked note, optional assignee, area (work, health, home, personal, learning, finance), energy level (high, medium, low), tags, comments, and creation date. Tasks support recurring schedules and `blocked_by` dependency tracking. Tasks can be organized on a Kanban board grouped by status. |
